...
void Game::mouseDown(ImVec2 &location, Entity *entity)
{
	...
	if (entity && entity->getEntityType() == Entity::EntityBit)
	{
        ...
        if ((_dragBit->gameTag() < 128 && currentPlayerNumber != 0) || 
            (_dragBit->gameTag() >= 128 && currentPlayerNumber != 1)) {
            _dragBit = nullptr; // piece can't be selected if it's not the current player's turn
            return;
        }
	}
...
void Game::mouseUp(ImVec2 &location, Entity *entity)
{
	...
	if (_dragBit)
	{
		if (_dragMoved)
		{
			...
			// Is the move legal?
			if (_dropTarget && _dropTarget->canDropBitAtPoint(_dragBit, _dragBit->getPosition()))
			{
				// ------------------------------- update the half moves before moving the piece
				updateHalfMoves(*_dragBit, *_oldHolder, *_dropTarget);
				// ------------------------------- and now move the piece
				_dropTarget->dropBitAtPoint(_dragBit, _dragBit->getPosition());
